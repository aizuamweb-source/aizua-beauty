...
async function analyzeWithClaude(campaigns: Campaign[]): Promise<{
  ...
  const campaignsText = campaigns.map(c =>
    `- "${c.name}": ROAS ${c.roas_3d || c.purchase_roas.toFixed(2)}x, Gasto €${c.spend}, Budget €${c.daily_budget}/día, CTR ${(c.ctr * 100).toFixed(2)}%, ${c.impressions.toLocaleString()} impresiones, Estado: ${c.status}`
  ).join("\n");
