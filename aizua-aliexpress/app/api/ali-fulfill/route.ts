...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1b. Verificar límite de gasto diario
    const MAX_DAILY = parseFloat(process.env.MAX_DAILY_SPEND_EUR || "500");
    const today = new Date().toISOString().split("T")[0];
    const { data: spend } = await supabase
      .from("daily_spend")
      .select("amount_eur, order_count")
      .eq("date", today)
      .single();

    if (spend && spend.amount_eur >= MAX_DAILY) {
      console.error(`[ali-fulfill] Límite diario alcanzado: ${spend.amount_eur}€ / ${MAX_DAILY}€`);
        ...
        await fetch(
          `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,
          {
            ...
            body: JSON.stringify({
              ...
              text: `🚨 *LÍMITE GASTO DIARIO ALCANZADO*\n\nGasto hoy: €${spend.amount_eur} / €${MAX_DAILY}\nPedidos: ${spend.order_count}\n\n⚠️ Pedido *${order.order_number}* NO enviado a AliExpress. Revisa manualmente.`,
              parse_mode: "Markdown",
            }),
          }
        ).catch(console.error);
      }
      return NextResponse.json({ error: "daily_limit_reached", limit: MAX_DAILY, spent: spend.amount_eur }, { status: 429 });
    }
    ...
    // 6. Actualizar Supabase + registrar gasto diario
    await Promise.all([
      ...
      supabase.rpc("increment_daily_spend", { amount: order.total }),
    ]);
